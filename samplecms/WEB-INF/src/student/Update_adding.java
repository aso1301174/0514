...
public class Update_adding extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		try {
			...
			//SQL文設定の準備・SQL文の実行
			String insertSql = "insert into situation values (?,?);";
			stmt = conn.prepareStatement(insertSql);
			stmt.setString(1,l_id);
			stmt.setString(2,t_id);
			stmt.executeUpdate();

			String selectSql2 = "select lo.log_id , lo.log_name , te.test_name , te.class from test te, situation si, login lo WHERE si.log_id = lo.log_id AND te.test_id = si.test_id AND lo.log_id = ?;";
			...
			RequestDispatcher rd = request
					.getRequestDispatcher("./student_home");
